...
def get_loader(config, dataset_name='Shoes', mode='train', num_workers=1):
    """Build and return a data loader."""
    if dataset_name=='Shoes':
        dataset = ShoesDataset(config, mode, dataset_name)
        if mode=='train':
...
class ShoesDataset(DatasetBase):
    ...
    def read_dataset(self):
        ...
        if self.mode in {'train','test'}:
            ## ids=[train_id, valid_id] if self.model in ['train', 'valid'] else ids=[test_id]
            self.ids=ids[0]
        else:
            self.ids = ids[1]
        ...
        with open(labels_filepath, 'rb') as f:
            ...
            ## self.labels is a dictionary. example:  {'./data/train/adidas/40.jpg':0 ,'./data/train/converse/40.jpg':1,..... }
            f.close()
